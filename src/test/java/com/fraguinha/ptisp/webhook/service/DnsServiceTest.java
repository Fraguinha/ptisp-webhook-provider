...
class DnsServiceTest {
    ...
    void setUp() {
        ...
        this.dnsService = new DnsService(List.of(account), new SimpleMeterRegistry());
    }
    ...
    @Test
    void getAllRecordsFailsLoudlyOnApiFailure() {
        Mockito.when(this.client.listRecords("example.com")).thenThrow(new RuntimeException("connection refused"));

        Assertions.assertThrows(PtispApiException.class, () -> this.dnsService.getAllRecords());
    }

    @Test
    void getAllRecordsFailsLoudlyWhenResultNotOk() {
        Mockito.when(this.client.listRecords("example.com"))
    ...
    @Test
    void getAllRecordsFailsLoudlyWhenBodyMissing() {
        Mockito.when(this.client.listRecords("example.com")).thenReturn(Optional.empty());

        Assertions.assertThrows(PtispApiException.class, () -> this.dnsService.getAllRecords());
    }

    @Test
    void getAllRecordsFailsWhenAnySingleDomainFails() {
        final PtispClient healthy = Mockito.mock(PtispClient.class);
        Mockito.when(healthy.listRecords("healthy.com"))
                .thenReturn(Optional.of(new PtispDTO.ListResponse("ok", List.of(
                        new PtispDTO("1", "a.healthy.com.", "A", "IN", 300L, "1.1.1.1", "", "")))));
        Mockito.when(this.client.listRecords("example.com")).thenThrow(new RuntimeException("boom"));

        final DnsService service = new DnsService(List.of(
                new ManagedAccount(List.of("healthy.com"), healthy),
                new ManagedAccount(List.of("example.com"), this.client)), new SimpleMeterRegistry());

        Assertions.assertThrows(PtispApiException.class, service::getAllRecords);
    }

    @Test
    void getManagedDomainsListsEveryConfiguredDomain() {
        final PtispClient other = Mockito.mock(PtispClient.class);
        final DnsService service = new DnsService(List.of(
                new ManagedAccount(List.of("b.com", "a.com"), this.client),
                new ManagedAccount(List.of("a.com", "c.com"), other)), new SimpleMeterRegistry());

        Assertions.assertEquals(List.of("a.com", "b.com", "c.com"), service.getManagedDomains());
    }

    @Test
    void updateRecordRecreatesTargetWhenOnlyTtlChanges() {
            final PtispDTO existing = new PtispDTO("7", "test.example.com.", "A", "IN", 300L, "1.1.1.1", "", "");
            final PtispDTO owner = new PtispDTO("8", "_edns.a-test.example.com.", "TXT", "IN", 300L, null, null,
                            "\"heritage=external-dns\"");
            Mockito.when(this.client.listRecords("example.com"))
                            .thenReturn(Optional.of(new PtispDTO.ListResponse("ok", List.of(existing, owner))))
                            .thenReturn(Optional.of(new PtispDTO.ListResponse("ok", List.of(owner))));

            this.dnsService.updateRecord(
                            ExternalDnsDTO.of("test.example.com", List.of("1.1.1.1"), "A", 300L),
                            ExternalDnsDTO.of("test.example.com", List.of("1.1.1.1"), "A", 60L));

            Mockito.verify(this.client).deleteRecord("example.com", "7");
            Mockito.verify(this.client).addRecord(ArgumentMatchers.eq("example.com"),
                            ArgumentMatchers.argThat(r -> r.ttl() == 60L));
    }

    @Test
    void updateRecordDoesNothingWhenTargetsAndTtlUnchanged() {
            this.dnsService.updateRecord(
                            ExternalDnsDTO.of("test.example.com", List.of("1.1.1.1"), "A", 300L),
                            ExternalDnsDTO.of("test.example.com", List.of("1.1.1.1"), "A", 300L));

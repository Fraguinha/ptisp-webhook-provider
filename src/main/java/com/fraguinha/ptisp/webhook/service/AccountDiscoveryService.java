...
public class AccountDiscoveryService {
        ...
        public List<ManagedAccount> discoverAccounts(final Map<String, String> env, final RestClient.Builder builder) {
                ...
                return env.entrySet().stream()
                                ...
                                .flatMap(e -> {
                                        final String prefix = e.getKey().substring(0,
                                                        e.getKey().length() - "_DOMAINS".length());
                                        final String email = env.get(prefix + "_EMAIL");
                                        final String hash = env.get(prefix + "_HASH");
                                        if (email == null || hash == null) {
                                                AccountDiscoveryService.log.warn("Skipping {}: missing {} env var",
                                                                prefix,
                                                                email == null ? prefix + "_EMAIL" : prefix + "_HASH");
                                                return java.util.stream.Stream.<ManagedAccount>empty();
                                        }
                                        ...
                                        AccountDiscoveryService.log.info("Registered PTISP account: {} for domains {}",
                                                        email, domains);
                                        return java.util.stream.Stream.of(
                                                        new ManagedAccount(domains, new PtispClient(email, client)));
                                })
                                .toList();
        }
}

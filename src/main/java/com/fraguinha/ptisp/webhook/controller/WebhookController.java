...
public class WebhookController {
    ...
    public ResponseEntity<Map<String, List<String>>> negotiate() {
        final List<String> domains = this.dnsService.getManagedDomains();
        WebhookController.log.debug("ExternalDNS negotiation, advertising {} managed domains", domains.size());
        return ResponseEntity.ok()
                .header(WebhookConstants.PROVIDER_NAME_HEADER, WebhookConstants.PROVIDER_NAME)
                .body(Map.of("include", domains));
    }

    @GetMapping(value = "/records", produces = WebhookConstants.CONTENT_TYPE)
    public ResponseEntity<List<ExternalDnsDTO>> getRecords() {
        WebhookController.log.debug("ExternalDNS fetching records");
        return ResponseEntity.ok()
                .header(WebhookConstants.PROVIDER_NAME_HEADER, WebhookConstants.PROVIDER_NAME)
    ...
    public ResponseEntity<Void> applyChanges(@RequestBody final ExternalDnsDTO.Changes changes) {
        final int creates = changes.create().stream().mapToInt(e -> e.targets().size()).sum();
        final int updates = changes.updateOld().size();
        final int deletes = changes.delete().stream().mapToInt(e -> e.targets().size()).sum();

        if (creates > 0 || deletes > 0) {
            WebhookController.log.info("Applying DNS changes: create={}, update={}, delete={}", creates, updates, deletes);
        } else {
            WebhookController.log.debug("Applying DNS changes: create={}, update={}, delete={}", creates, updates, deletes);
        }

        changes.delete().forEach(this.dnsService::deleteRecord);

        for (int i = 0; i < changes.updateOld().size(); i++) {
            this.dnsService.updateRecord(changes.updateOld().get(i), changes.updateNew().get(i));
        }
